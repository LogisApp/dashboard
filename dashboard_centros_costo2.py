...
@st.cache_data
def load_data(file):
    # Intenta leer la primera hoja con encabezados conocidos o infiere saltos
    ...
    for c in list(df.columns):
        if c in colmap:
            continue
        # intentos de mapeo por similitud simple
        lc = c.lower()
        if "centro" in lc and "costo" not in df.columns:
            colmap[c] = "centro"
        if "descrip" in lc:
            colmap[c] = "descripcion"
        if "valor" in lc or "monto" in lc or "importe" in lc:
            colmap[c] = "valor"
        if "nombre" in lc:
            colmap[c] = "nombre"
    df = df.rename(columns=colmap)
    # Filtra filas con columnas clave faltantes
...
if uploaded:
    try:
        ...
        with st.sidebar:
            ...
            if miny == maxy:
                yr = st.slider("Año", min_value=miny-1, max_value=maxy+1, value=(miny, maxy))
            else:
